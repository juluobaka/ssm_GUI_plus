...
func (c *Client) ExecuteCommand(d *Device, command string) (string, error) {
	conn, err := c.Connect()
	if err != nil {
		return "", err
	}
	...
	if err := conn.Run(fmt.Sprintf("host:transport:%s", d.serial)); err != nil {
		return "", err
	}
	...
	// For shell commands, we need to read until EOF or timeout
	if strings.HasPrefix(command, "shell:") {
		var out bytes.Buffer
		buf := make([]byte, 4096)
		for {
			// Set a short deadline for reading shell output
			...
			if err != nil {
				...
				// If it's a timeout, we assume the command is running in background or finished
...
func (c *Client) Devices() ([]*Device, error) {
	...
	for l := range strings.Lines(resp) {
		...
		fields := strings.Fields(l)
		if len(fields) < 2 || len(fields[0]) == 0 || fields[0] == "List" {
			continue
		}
